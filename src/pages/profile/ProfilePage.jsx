    ...
    <div className='mx-auto w-full max-w-6xl p-3 sm:p-4'>
      ...
      <style>{`
        @keyframes subtle-glow {
          ...
          50% {
            box-shadow: 0 4px 30px rgba(34, 197, 94, 0.25);
          }
        }
          ...
          {getStatus.isLoading ? (
            ...
            <div>
              <div className='mt-4 rounded-2xl bg-white px-3 py-4 shadow-lg sm:mt-6 sm:rounded-3xl sm:px-4 sm:py-6 md:px-6'>
                ...
                <div className='mt-4 flex flex-col gap-2 sm:flex-row sm:gap-3'>
                  <Skeleton.Button active size='large' block className='!h-10 !rounded-xl' />
                  <Skeleton.Button active size='large' block className='!h-10 !rounded-xl' />
                </div>
              </div>
              <div className='mt-4 rounded-2xl bg-white px-3 py-4 shadow-lg sm:mt-6 sm:rounded-3xl sm:px-4 sm:py-6 md:px-6'>
            ...
            <div className='mt-4 space-y-4 sm:mt-6'>
              {getStatus.data.map((modul) => (
                <div
                  key={modul.id}
                  className={`group overflow-hidden rounded-2xl shadow-lg transition-all duration-300 hover:shadow-xl sm:rounded-3xl ${
                    modul.successExam
                      ? 'success-module bg-gradient-to-br from-white to-green-50/30'
                      : 'bg-white'
                  }`}
                >
                  {/* Header Section */}
                  <div
                    className={`relative z-[1] border-b px-4 py-4 sm:px-6 sm:py-5 ${
                      modul.successExam
                        ? 'border-green-100 bg-gradient-to-r from-green-50/50 to-white'
                        : 'border-gray-100 bg-gradient-to-r from-blue-50 to-white'
                    }`}
                  >
                    <div className='flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between'>
                      <div className='flex-1'>
                        <div className='mb-2 flex items-center gap-2'>
                          ...
                          {modul.successExam && (
                            <span className='success-badge ml-2 inline-flex items-center gap-1 rounded-full bg-gradient-to-r from-green-500 to-emerald-600 px-2 py-0.5 text-xs font-bold text-white shadow-md sm:px-3 sm:py-1'>
                              <Check size={12} />
                              A`LO
                            </span>
                          )}
                      ...
                      {/* Status Badge */}
                      <div
                        className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-semibold sm:px-4 sm:py-2 sm:text-sm ${
                          modul.successExam
                            ? 'border-green-200 bg-green-50 text-green-700'
                            : getStatusColor(modul.moduleState)
                        }`}
                      >
                  ...
                  <div className='relative z-[1] px-4 py-4 sm:px-6 sm:py-5'>
                    {/* Progress Section */}
                    <div className='mb-2'>
                      <div className='mb-0 flex items-center justify-between sm:mb-2'>
                        <span className='text-sm font-semibold text-gray-700 sm:text-base'>
                          Tugallanish foizi
                        </span>
                        <span
                          className={`text-lg font-bold sm:text-xl ${
                            modul.successExam ? 'text-green-600' : 'text-blue-600'
                          }`}
                        >
                    ...
                    <div className='mb-4'>
                      <div className='mb-0 flex items-center justify-between sm:mb-2'>
                        ...
                        >
                          {modul.moduleTestResult || 0}%
                        </span>
                      </div>
                      <Progress
                        percent={modul.moduleTestResult || 0}
                        strokeWidth={12}
                        strokeColor={
                          modul.successExam
                            ? {
                    ...
                    <div className='flex flex-col gap-2 sm:flex-row sm:gap-3'>
                      {activeTab === 'COMPLETED' ? (
                        ...
                        <Button
                          ...
                          iconPosition='end'
                          className='flex-1 rounded-xl border-blue-200 bg-blue-50 !py-1 font-semibold text-blue-700 transition-all hover:!border-blue-300 hover:!bg-blue-100 hover:!text-blue-800'
                        >
                          Yakunlash
                        </Button>
                      )}
