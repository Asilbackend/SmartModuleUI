...
import { getAllQuestionsbyId, postFinishTest, postSubmitTest } from 'src/api/test-page.api';

const OptionButton = ({ option, idx, userAnswer, correctAnswer, onSelect, isFinished }) => {
  const isCorrect = option.questionOptionId === correctAnswer;
  const isSelected = option.questionOptionId === userAnswer;
  ...
  return (
    <button
      onClick={() => onSelect(option.questionOptionId)}
...
const duration = 600; // 10 daqiqa (600 soniya)
...
const Test = () => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      return res.data.map((q) => ({
        ...
        correctAnswer: q.options.find((o) => o.correct)?.questionOptionId,
      }));
    },
  });

  //Tanlangan Javobni yuborish uchun mutation
  const submitTest = useMutation({
    mutationFn: async (questionOptionId) => {
      const res = await postSubmitTest(questionOptionId);
      return res.data;
    },
  });

  //Testni tugatish uchun mutation
  const finishTest = useMutation({
    mutationFn: async (moduleId) => {
      const res = await postFinishTest(moduleId);
      return res.data;
    },
    onSuccess: () => {
      setIsFinished(true);
  ...
  // Vaqt tugaganda yoki test tugaganda
  useEffect(() => {
    if (timeLeft <= 0 && !isFinished) {
      // Vaqt tugadi - testni avtomatik yakunlash
      finishTest.mutate(moduleId);
      return;
    }
    if (isFinished) {
      return;
  ...
  }, [timeLeft, isFinished, moduleId, finishTest]);
  ...
  const handleAnswer = (optionId) => {
    ...
    // Har bir javobni tanlaganda API ga yuborish
    submitTest.mutate(optionId);
  };
  ...
  let correctCount = 0;
  if (isFinished) {
    correctCount = questions.filter((q, idx) => selectedAnswers[idx] === q.correctAnswer).length;
  }
        ...
        <div className='hidden border-b border-gray-100 p-4 sm:p-5 md:block'>
          <div className='flex flex-wrap gap-2'>
            {questions.map((q, idx) => {
              ...
              const isCorrectAnswer = isFinished && answered === q.correctAnswer;
              const isWrongAnswer = isFinished && answered && answered !== q.correctAnswer;
      ...
      {currentQuestion && (
        <div className='overflow-hidden rounded-2xl bg-white shadow-lg sm:rounded-3xl'>
          <div className='border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white p-4 sm:p-6'>
            ...
            {isFinished && userAnswer === currentQuestion.correctAnswer && (
            ...
            {isFinished && userAnswer && userAnswer !== currentQuestion.correctAnswer && (
                ...
                <p className='text-xs font-medium text-red-800 sm:text-sm'>
                  Noto`g`ri javob. To`g`ri javob:{' '}
                  {
                    currentQuestion.options.find(
                      (o) => o.questionOptionId === currentQuestion.correctAnswer
                    )?.name
                  }
                </p>
              </div>
            ...
            <div className='flex items-center justify-between gap-3'>
              ...
              {!isFinished ? (
                <Button
                  ...
                  loading={finishTest.isPending}
                  size='large'
                  className='!rounded-xl bg-gradient-to-r from-red-500 to-red-600 !px-5 !py-5 !text-sm font-bold shadow-md transition-all hover:!from-red-600 hover:!to-red-700 hover:shadow-lg sm:!px-6 sm:!text-base'
                >
                  Testni yakunlash
                </Button>
              ) : (
          ...
          itemRender={(page, type, originalElement) => {
            if (type === 'page') {
              ...
              const isCorrectAnswer =
                isFinished && selectedAnswers[page - 1] === questions[page - 1].correctAnswer;
              const isWrongAnswer =
                isFinished &&
                selectedAnswers[page - 1] &&
                selectedAnswers[page - 1] !== questions[page - 1].correctAnswer;
